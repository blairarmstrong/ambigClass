...
for k in defsspace:
    ...
    if not os.path.isfile('./critwindows/'+k+'.mat'):
        ...
        for m in defsspace[k]:
            if len(def_arr.shape) > 1 and len(def_arr) > 0:
...
#                print(def_arr[i])
            ...
            elif len(def_arr.shape) == 1 and len(def_arr) == 0:
                cossim = 0;
                #arr[int(m)-1, i] = cossim
            else:
                #consider if you should remove this functionality; you probably
                #don't even want to keep the data from cases where you only ahve
                #one example vector
                    cossim = cosine_similarity(defsspace[k][m][0], def_arr)
                    arr[int(m)-1, i] = cossim                
